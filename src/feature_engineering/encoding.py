...
#  [ColumnDropper class: drops unnecessary columns]
...
class EncodingTransformer(BaseEstimator, TransformerMixin):
    ...
    Compatible with real_distance_km from updated feature extraction.
    """
    def __init__(self):
    ...
    def fit(self, X, y=None):
        # apply cyclic encoding first, then fit the rest of the columns
        X_temp = self.cyclic_transformer.transform(X.copy())

        # correctly define columns for ColumnTransformer after cyclic encoding
        ordinal_cols = [col for col in self.ordinal_cols if col in X_temp.columns]
        onehot_cols = [col for col in self.onehot_cols if col in X_temp.columns]
...
if __name__ == "__main__":
    print("Encoding module updated for real_distance_km pipeline compatibility")